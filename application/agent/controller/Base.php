...
class Base extends Common{
    ...
    protected function _initialize(){
        ...
        if(__JINTAO_CONTROLLER__ != 'Passport'){
			...
			$this->admin = Db::name('copy_province')->where(array('user_id'=>$this->uid))->find();
			$this->roleName = $this->admin['name'].'省代理';
			$this->role = 1;
			$this->province = $this->admin['id'];
			if(!$this->admin){
				$this->admin = Db::name('city')->where(array('user_id'=>$this->uid))->find();
				$this->roleName = $this->admin['name'].'市代理';
				$this->role = 2;
				$this->city = $this->admin['city_id'];
				if(!$this->admin){
					$this->admin = Db::name('area')->where(array('user_id'=>$this->uid))->find();
					$this->roleName = $this->admin['area_name'].'区代理';
					$this->role = 3;
					$this->area = $this->admin['area_id'];
					if(!$this->admin){
						header("Location: " . url('passport/login'));
						die;
					}
