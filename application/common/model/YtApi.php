...
class YtApi extends Base{
    ...
    public function getExpressList11($data){
        ...
        $d1 = strstr($data['sender_address'],$data['sender_province']);
        if($d1 == false){
            $s_address = $data['sender_address'];
        }else{
            $s_address = str_replace($data['sender_province'].$data['sender_city'].$data['sender_area'],"",$data['sender_address']);
        }
        $content['senderLocation']=$s_address;
        ...
        $d2 = strstr($data['recipients_address'],$data['recipients_province']);
        if($d2 == false){
            $r_address = $data['recipients_address'];
        }else{
            $r_address = str_replace($data['recipients_province'].$data['recipients_city'].$data['recipients_area'],"",$data['recipients_address']);
        }
        $content['receiveLocation']=$r_address;
        ...
        if($performance['code'] == 0){
            ...
            foreach($performance['result'] as $k=>$v){
                $c = Db::name('express_cate')->where(array('cate_name'=>$v['tagType'],'type'=>9,'firstPrice'=>0))->find();
                if(!$c){
                    $c = Db::name('express_cate')->where(array('pinyin2'=>$v['tagCode'],'type'=>9,'firstPrice'=>0))->find();
                    if(!$c && $data['type']==2){
                        $cate_name = cut_msubstr($v['channel'],0,2,true);
                        $c = Db::name('express_cate')->where(array('cate_name'=>$cate_name,'type'=>9,'firstPrice'=>0))->find();
                    }
                }
                ...
                if($c){
                    ...
                    if($pag == 1){
                        //加起来价格不对
                        $getCatePrice = model('Setting')->getCatePrice($data['uid'],$data['totalWeight'],$totalFreight,0,0,$v['originalPrice']*100,0,$c);
                    }else{
                        $getCatePrice = model('Setting')->getCatePrice($data['uid'],$data['totalWeight'],$totalFreight,$priceA*100,$priceB*100,$v['originalPrice']*100,$other,$c);
                    }
                    $expressList[$k]['getCatePrice'] = $getCatePrice;
                    $expressList[$k]['discount'] =$getCatePrice['discount'];
